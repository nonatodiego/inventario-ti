...
export const inventoryRouter = router({
  ...
  create: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Always pass detail objects if resource is marked as present
  ...
  getHistoryByType: publicProcedure
    .input(z.object({ licenseType: z.enum(["E1", "E3"]) }))
    .query(async ({ input }) => {
      return await getLicenseHistoryByType(input.licenseType);
    }),

  transferLicense: publicProcedure
    .input(
      z.object({
        inventoryId: z.number(),
        newUserName: z.string().min(1),
        newLicenseType: z.enum(["E1", "E3"]),
      })
    )
    .mutation(async ({ input }) => {
